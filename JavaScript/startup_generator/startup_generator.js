...
var ideaGenerator = function () {
  var randomIdeaOne = thisArray[Math.floor(Math.random() * thisArray.length)];
  var randomIdeaTwo = thatArray[Math.floor(Math.random() * thatArray.length)];
  var randomIdea = randomIdeaOne + '-' + randomIdeaTwo;
  return (randomIdea);
}

var choice = prompt('Generate new idea?');
if (choice === 'Yes' || choice === 'yes') {
  console.log (ideaGenerator());
}
else if (choice === 'No' || choice === 'no') {
   prompt('Give us your creative ideas!');
   alert('Thanks!');
}
// else {
